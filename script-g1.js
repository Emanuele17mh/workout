document.addEventListener('DOMContentLoaded', function() {
    ...
    buttons.forEach(button => {
        ...
        if (localStorage.getItem(`exerciseDone${exerciseId}`) === 'true') {
            markAsDone(row, button);
        }

        // Aggiungi l'evento click al pulsante
        button.addEventListener('click', function() {
            const isDone = localStorage.getItem(`exerciseDone${exerciseId}`) === 'true';
            if (!isDone) {
                localStorage.setItem(`exerciseDone${exerciseId}`, 'true');
                markAsDone(row, button);
            }
        });
    });
    ...
    resetButton.addEventListener('click', function() {
        buttons.forEach(button => {
            const row = button.closest('tr');
            const exerciseId = row.getAttribute('data-exercise');
            localStorage.removeItem(`exerciseDone${exerciseId}`); // Rimuovi lo stato salvato
